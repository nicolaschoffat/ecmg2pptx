    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        for i, node in enumerate(nodes):
            ...
            for img_el in screen.findall("image"):
                ...
                if img_file.startswith('@/'):
                    img_file = img_file.replace('@/', '', 1)
                # 🔍 Recherche de l’image dans l’arborescence
                image_path = None
                for root, _, files in os.walk(tmpdir):
                    if os.path.basename(img_file) in files:
                        image_path = os.path.join(root, os.path.basename(img_file))
                        break
                if os.path.exists(image_path):
                    ...
                    st.warning(f"❌ Image non trouvée : {img_file} (chemin : {image_path})")
                    try:
                else:
                    st.warning(f"❌ Image non trouvée : {img_file} (chemin : {image_path})")
                    st.warning(f"❌ Image non trouvée : {img_file} (chemin : {image_path})")
